...
export default function CVPage() {
  ...
  return (
    <div className={dark ? 'dark' : ''}>
      <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-100 text-slate-800 dark:from-slate-900 dark:via-slate-950 dark:to-slate-950 dark:text-slate-100">
        <header className="sticky top-0 z-20 border-b border-slate-200/60 bg-white/70 backdrop-blur supports-[backdrop-filter]:bg-white/60 dark:border-slate-800 dark:bg-slate-900/70">
          <div className='mx-auto flex max-w-4xl items-center justify-between px-4 py-3'>
            ...
            <div className='flex items-center gap-2 text-sm'>
              <button onClick={() => window.print()} className='rounded-xl border border-slate-200/70 px-3 py-1.5 hover:bg-slate-100' aria-label='Print CV'>
                Print
              </button>
              <button
                onClick={() => setDark((d) => !d)}
                className="rounded-xl border border-slate-200/70 bg-white/70 px-3 py-1.5 shadow-sm backdrop-blur hover:bg-white transition dark:border-slate-700 dark:bg-slate-800"
        ...
        <section className='mx-auto max-w-4xl px-4 py-10 sm:py-12'>
          <div className='grid items-start gap-6 sm:grid-cols-[auto_1fr]'>
            <img src="portrait.jpg" alt="Portrait of Mads Dixen" className="h-72 w-72 rounded-2xl object-cover shadow-lg" />
            <div>
        ...
        <section className='mx-auto max-w-4xl px-4 py-4'>
          ...
          <div className='mt-3 flex flex-wrap gap-2'>
            {skills.map(s => (
              <span key={s} className="rounded-full bg-white/70 backdrop-blur px-3 py-1 text-sm border border-slate-200/60 shadow-sm hover:shadow transition dark:bg-slate-900/60 dark:border-slate-700">
                {s}
              </span>
            ))}
          </div>
        ...
        <section className='mx-auto max-w-4xl px-4 py-8'>
          ...
          <div className='mt-4 space-y-6'>
            {experience.map((item, idx) => (
              <article key={idx} className="group relative overflow-hidden rounded-2xl bg-white border border-slate-200/70 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg dark:bg-slate-900 dark:border-slate-800">
        ...
        <section className='mx-auto max-w-4xl px-4 pb-16'>
          ...
          <div className='mt-4 grid gap-4 sm:grid-cols-2'>
            {education.map((e, i) => (
              <div key={i} className="relative overflow-hidden rounded-2xl border border-slate-200/70 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg dark:bg-slate-900 dark:border-slate-800">
        ...
        <footer className="border-t border-slate-200/70 bg-gradient-to-b from-white to-slate-100 py-6 text-sm text-slate-500 dark:border-slate-800 dark:from-slate-900 dark:to-slate-950 dark:text-slate-400">
